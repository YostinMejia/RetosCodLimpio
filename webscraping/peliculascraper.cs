...
namespace webscraping
{
    internal class pelicula
    {
        ...
        public static List<List<string>> ComentariosUsuarios(string url, bool pelicula_true = true)
        {
            ...
            try
            {
                ...
                foreach (var comentario in parse_data)
                {
                    ...
                    datos.Add(new List<string> { nombre, TrimDato(comentario_txt) });
        ...
        public static List<List<string>> Calificacion(string html, bool pelicula_true = true)
        {
            HtmlDocument document = new HtmlDocument();
            document.LoadHtml(html);
            ...
            var score = informacion.Descendants("score-board").Where(node => node.GetAttributeValue("class", "").Contains("scoreboard")).ToList()[0];
        ...
        public static List<string> DatosPelicula(string url)
        {
            ...
            List<List<string>> calificacion = Calificacion(response);
            List<List<string>> datos_princi = DatosPrincipales(response);
